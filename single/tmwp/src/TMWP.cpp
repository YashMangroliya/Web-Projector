...
send(clientSocketDescriptor,str,strlen(str),0);
}
void Response::close()
{
...
Request *request=new Request;    // v v v v v v Important
...
this->ptrOnRequest(*request,response); // request is a pointer of Request class
...
// check if request object contains anything against property that got set because of call to forward
// if yes reprocess as if the request has arrived for the value of the property that got set because of call to forward method
...
} //infinite loop to enable forwarding feature ends here 
